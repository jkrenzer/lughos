...
//   if(voltage_min>f) return 0;
  ...
  std::string answer = this->inputOutput("\x00U"+request+"\r",boost::regex("A\\w\\w\\w\\w"));
...
//   if(voltage_max<f) return 0;
  ...
  stream << std::hex << (int) unitsToVoltage.yToX(f);
  std::string request= stream.str();
 std::string answer = this->inputOutput("\x00M"+request+"\r",boost::regex("B\\w\\w\\w\\w"));
...
float RFG::set_current_lim(float  f)
{
  ...
  stream << std::hex << f;
  std::string request= stream.str();
  std::string answer = this->inputOutput("\x00I"+request+"\r",boost::regex("C\\w\\w\\w\\w"));
...
int RFG::set_power_lim(float f)
{
  ...
  stream << std::hex << f;
  std::string request= stream.str();
  std::string answer = this->inputOutput("\x00P"+request+"\r",boost::regex("D\\w\\w\\w\\w"));
