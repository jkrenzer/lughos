...
namespace lughos 
{
  ...
  public:
	...
	this->init();
      ...
      {
	this->addWidget(new Wt::WText(std::string("Port ") + dynamic_cast <const Connection< serialContext > * const>(coolpak->getConnection())->port_name + std::string(" cannot be opened. Port blocked, disabled or wrong portname.")));
      }
    }
